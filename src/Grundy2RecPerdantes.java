...
class Grundy2RecPerdantes {
    ...
    boolean estConnu(ArrayList<Integer> essai, ArrayList<ArrayList<Integer>> sitSaved) {
        ...
        // Partant du principe que dans sitSaved, les essais sont tous simplifiés
        ArrayList<Integer> essaiClean = simplifier(essai);

        int i = 0;
        while (i < sitSaved.size() && !ret) { // Parcours la liste des situations
            if (sitSaved.get(i).equals(essaiClean)) { // Si l'essai est trouvé
                ret = true;
            }
            i++;
        }
    ...
    void ajouterEssai(ArrayList<Integer> essai, ArrayList<ArrayList<Integer>> sitSaved) {
        if (!estConnu(essai, sitSaved)) { // Si l'essai n'est pas deja connu
        ...
        if (jeu == null) {
            ...
            while (ligne != -1 && !gagnant) {
                if (estPerdante(essai)) {
                    ...
                    ajouterEssai(essai, posPerdantes); // On sauvegarde la situation en tant que perdante
                ...
                if (estConnu(jeu, posPerdantes)) {
                    ...
                    while ((ligne != -1) && ret) {
                        ...
                        if (estPerdante(essai)) {
                            ...
                            ajouterEssai(essai, posPerdantes);
                            ret = false;
